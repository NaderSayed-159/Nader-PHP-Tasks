...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $tmpPath = $_FILES["cvFile"]["tmp_name"];


    //cv Validation

    if (empty($cvFilename)) {
        echo "<h2 class='text-center text-danger'>Please choose the uploade your CV!</h2> <br>";
    } else {

        //get the file extension
        $fileArray = explode(".", $cvFilename);
        $fileExtension =  strtolower($fileArray[1]);

        //rename the file
        $randomName = rand() . time() . "." . $fileExtension;

        //check type

        if ($fileExtension == "pdf") {
            $disFolder = "./uploads/";
            $disPath = $disFolder . $randomName;

            if (move_uploaded_file($tmpPath, $disPath)) {

                echo "<h2 class='text-center text-sussess'>CV Uploaded!</h2> <br>";
            } else {
                echo "<h2 class='text-center text-danger'>Please try to upload again!</h2> <br>";
            }
        } else {
            echo "<h2 class='text-center text-danger'>Cv should be PDF file!</h2> <br>";
        }
    }
...
<body>
    <form action="<? echo stripcslashes($_SERVER["PHP_SELF"]); ?>" class="container p-5 bg-dark mt-5" method="post" enctype="multipart/form-data">
        ...
        <div class="form-floating">
            <input type="text" class="form-control mt-3" id="loatingage" placeholder="Age" name="age">
            <label for="floatingage">Age</label>
        </div>
        <div class="form-floating">
            <input type="url" class="form-control mt-3" id="floatingurl" placeholder="Linkedin" name="linkedin">
            <label for="floatingurl">Linkedin URL</label>
        </div>
        <div class="form-group">
            <input type="file" class="form-control mt-3" id="floatingPassword" placeholder="Linkedin" name="cvFile">
        </div>
